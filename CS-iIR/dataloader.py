...
class Dataset():
    def __init__(self, dataset, sep='\t', header_name=['u', 'i', 'r', 't', 'm'], pos_type=[4.0, 5.0], num_his_semester=9,
                 num_cur_semester=1, genre=18):
        self.pos_type = pos_type
        self.num_his_semester = num_his_semester
        self.num_cur_semester = num_cur_semester
        self.genre = genre
        ...
        self.his_month_list = [i for i in range(0, 0 + num_his_semester)]
        self.cur_month_list = [i for i in range(num_his_semester, num_his_semester + num_cur_semester)]
        self.train_month_list = [i for i in range(0, num_his_semester + num_cur_semester)]
    ...
    def get_uesr_sim_mat(self):
        ...
        user_mat = np.zeros(norm_user_mat.shape)
        for i in range(norm_user_mat.shape[0]):
            max_index = np.argmax(norm_user_mat[i])
            user_mat[i, max_index] = norm_user_mat[i, max_index]
        return user_mat
