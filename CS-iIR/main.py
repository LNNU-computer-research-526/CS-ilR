...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='learning_platform', help='The name of dataset.')
    ...
    parser.add_argument('--num_his_semester', type=int, default=3)
    parser.add_argument('--num_cur_semester', type=int, default=1)

    parser.add_argument('--decay_factor', type=float, default=1e-8, help='')
    parser.add_argument('--pri_factor', type=int, default=128, help='')
    parser.add_argument('--alphas', type=str, default='[0.2, 1.0, 0.2, 0.2]', help='')
    parser.add_argument('--use_user_si', action='store_false')
    parser.add_argument('--use_item_si', action='store_false')
    parser.add_argument('--user_threshold', type=float, default=0.6, help='')
    parser.add_argument('--item_threshold', type=float, default=0.6, help='')
    parser.add_argument('--article_genre', type=int, default=12, help='')
    args = parser.parse_args()
    print(args)

    dataloader = Dataset(args.dataset, num_his_semester=args.num_his_semester, num_cur_semester=args.num_cur_semester, genre=args.article_genre)
    ...
    user_sim_mat, item_sim_mat, decay_factor_mat = dataloader.get_user_item_sim_mat(args.use_user_si, args.user_threshold,
                                                                  args.use_item_si, args.item_threshold)

    lm = FR_sii(init_adj_mat, init_time_mat, decay_factor_mat, decay_factor=args.decay_factor, pri_factor=args.pri_factor,
              alpha_list=eval(args.alphas),
    ...
    precisions, recalls, f1_scores, mrrs, ndcgs = metric.calculate_all(user_truth_items_list,
                                                                       user_prediction_items_list, eval(args.topks))
    t3 = time.time()
    print('Test metrics:')
    for ind, topk in enumerate(eval(args.topks)):
        print('\t- Top{}:\tF1:{:.4f}\tMRR:{:.4f}\tNDCG:{:.4f}\tprecisions:{:.4f}\trecalls:{:.4f}'.format(topk, f1_scores[ind], mrrs[ind], ndcgs[ind], precisions[ind], recalls[ind]))
    print('Time info:')
