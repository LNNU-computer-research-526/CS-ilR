...
class FR_sii(object):
    def __init__(self, init_adj_mat, init_time_mat, decay_factor_mat, decay_factor=0, pri_factor=256, alpha_list=[0.2, 0.2, 0.2, 0.2],
                 use_user_si=True, use_item_si=True):
        self.his_adj_mat = init_adj_mat
        self.his_time_mat = init_time_mat
        self.decay_factor_mat = decay_factor_mat
    ...
    def train(self, cur_adj_mat, time_mat, cur_time, user_sim_mat=None, item_sim_mat=None):
        ...
        n = his_time_mat.shape[1]
        self.decay_factor_mat = np.tile(self.decay_factor_mat, (1, n))
        decay_mat = np.exp(np.multiply(self.decay_factor_mat, self.decay_factor*(his_time_mat - cur_time)))
